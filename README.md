...
- **方案策略**：文本水印默认用强鲁棒方案（WAM，抗裁剪/旋转/压缩，输出保持原分辨率锐利），**可切换经典 DWT**（密码 + 长度即可跨设备还原完整文本，但容量有限、不抗旋转）；Logo 水印用 DWT（完整还原，超大图先自动缩放至 1536px 内）
...
- **方案策略**：文本水印默认强鲁棒（WAM，任意图幅，抗裁剪/旋转/压缩），可切换经典 DWT（跨设备可还原完整文本，但容量有限、不抗旋转）；Logo 用 DWT（超大图先自动缩放至 1536px 内）
...
| 缩放 50% | ✅（自动还原回原尺寸后） | ✅ |
| 旋转 5° | ❌ | ✅（局部水印模式） |
| 裁剪（保留 75%） | ✅（自动填补回原尺寸后） | ✅（局部水印模式） |
| 裁剪 50% 以下 / 强噪声 | ❌ | ❌（技术边界，业界普遍） |

注 1：DWT 的缩放/裁剪抵抗依赖提取端的「还原/填补回原尺寸」网格再同步（App 自动尝试，与参考库攻击演示一致）；旋转是 DWT 的数学硬伤（参考库 README 的 45° 演示为挑选案例，实测还原后仍失败）。
注 2：WAM 文本水印现在用于**任意图幅**（嵌入在 256px、输出保持载体原分辨率锐利），实测 4000×3000 大图放大后仍可 0 误差提取。
