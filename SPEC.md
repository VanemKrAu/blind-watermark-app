...
- **自动提取防误判**：DWT 文本自动提取须通过**记录原文编码校验**（recBitsMatch，容错 ≤2 位——JPEG 轻损实测 1 位错；随机位流/假文本与固定文本编码差异远超容错，数学不可混）；**WAM 命中（conf ≥ 2 且记录 ≤4 位错）是强信号直接展示文本**（DWT 图/无痕图 conf 实测 <1 不会误命中；避免真 Logo 信号截胡 WAM 文本，优先级 手动 > WAM > DWT > 32 位码）；网格再同步候选 4 种（原图/拉伸/等比填充/内容原大小放回——裁剪攻击用「放回」恢复网格）
